...
export function ProductDetail() {
    ...
    const { addToCart, removeFromCart, cartList, total } = useCart();
    ...
    useEffect(() => {
        async function fetchProducts() {
            try {
                ...
                console.error(error.message, { closeButton: true, position: "bottom-center" });
            }
        }
        fetchProducts();
    }, [id]);
    ...
    useEffect(()=>{
        const productInCart = cartList.find(element=>element.id === product.id)
        if(productInCart){
            setInCart(true)
        }else{
            setInCart(false)
